							...
							gfp_t gfp_mask)
{
	...
	*(((__be16 *)th) + 6) = htons(((tcp_header_size >> 2) << 12) |
	...
	skb->tstamp.tv64 = 0;
	//xj:发送数据包，调用inet_connection_sock_af_ops->queue_xmit, 即ip_output的ip_queue_xmit函数
						   ...
						   int push_one, gfp_t gfp)
{
	...
	{
		unsigned int limit;
		//xj:TSO（TCP Segmentation Offload）分段
		...
		//xj:拥塞窗口 (cwnd，congestion window)
		cwnd_quota = tcp_cwnd_test(tp, skb);
		if (!cwnd_quota)
		{
			is_cwnd_limited = true;
