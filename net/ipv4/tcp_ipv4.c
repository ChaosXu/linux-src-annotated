...
static void tcp_v4_send_reset(struct sock *sk, struct sk_buff *skb)
{
	...
	{
		key = tcp_md5_do_lookup(sk, (union tcp_md5_addr *)&ip_hdr(skb)->saddr, AF_INET);
	}
 ...
 *	From tcp_input.c
 */
//xj:af_inet.c注册在net_protocol.handler中，由ip_local_deliver_finish调用
//tcp数据包接收处理
int tcp_v4_rcv(struct sk_buff *skb)
{
	...
	//xj:查找sock对象，应用层listen后才能找到？
	sk = __inet_lookup_skb(&tcp_hashinfo, skb, th->source, th->dest);
	if (!sk)
		//xj:没有sock跳转no_tcp_socket
		goto no_tcp_socket;
...
//xj:tcp协议的操作。被设置到sk->sk_prot，以便内核协议栈执行对用动作
