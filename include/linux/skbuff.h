...
struct sk_buff
{
	...
	union {
		...
		{
			/* These two members must be first. */
			struct sk_buff *next; //xj:下一块
			struct sk_buff *prev; //xj:上一块
...
#endif
	struct sock *sk;		//xj:内核socket
	struct net_device *dev; //xj:网络设备
