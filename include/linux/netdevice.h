...
#define NETDEV_FCOE_WWPN 1
	...
	RH_KABI_RENAME(int(*ndo_neigh_construct),
				   int(*ndo_neigh_construct_rh73))
	(struct neighbour *n);
	RH_KABI_RENAME(void(*ndo_neigh_destroy),
				   void(*ndo_neigh_destroy_rh73))
	(struct neighbour *n);

	RH_KABI_RENAME(int(*ndo_fdb_add),
				   int(*ndo_fdb_add_rh72))
	(struct ndmsg *ndm,
	 struct nlattr *tb[],
	 struct net_device *dev,
	...
	RH_KABI_USE_P(2, netdev_features_t (*ndo_features_check)(struct sk_buff *skb,
															 struct net_device *dev,
															 netdev_features_t features))
...
static inline bool __rh_has_ndo_setup_tc(const struct net_device *dev)
{
	const struct net_device_ops *ops = dev->netdev_ops;

	return (get_ndo_ext(ops, ndo_setup_tc_rh) ||
			ops->ndo_setup_tc_rh74 ||
			ops->ndo_setup_tc_rh72)
			   ? true
			   : false;
}
											  ...
											  bool more)
{
	...
	//xj：调用网卡驱动发送。比如ixgb_netdev_ops->ndo_start_xmit
...
__printf(3, 4) int netdev_printk(const char *level, const struct net_device *dev,
								 const char *format, ...);
__printf(2, 3) int netdev_emerg(const struct net_device *dev, const char *format, ...);
__printf(2, 3) int netdev_alert(const struct net_device *dev, const char *format, ...);
__printf(2, 3) int netdev_crit(const struct net_device *dev, const char *format, ...);
__printf(2, 3) int netdev_err(const struct net_device *dev, const char *format, ...);
__printf(2, 3) int netdev_warn(const struct net_device *dev, const char *format, ...);
__printf(2, 3) int netdev_notice(const struct net_device *dev, const char *format, ...);
__printf(2, 3) int netdev_info(const struct net_device *dev, const char *format, ...);
