...
static int igb_alloc_q_vectors(struct igb_adapter *adapter)
{
	...
	int rxr_remaining = adapter->num_rx_queues; //xj:读队列
	int txr_remaining = adapter->num_tx_queues; //xj:发队列
...
static void igb_setup_tx_mode(struct igb_adapter *adapter)
{
	...
	if (is_fqtss_enabled(adapter))
	{
		 ...
		 * CBS is disabled.
		 */
		max_queue = (adapter->num_tx_queues < I210_SR_QUEUES_NUM) ? adapter->num_tx_queues : I210_SR_QUEUES_NUM;
	...
	netdev_dbg(netdev, "FQTSS %s\n", (is_fqtss_enabled(adapter)) ? "enabled" : "disabled");
}
								...
								struct tc_cls_flower_offload *cls_flower)
{
	...
	hlist_for_each_entry(filter, &adapter->cls_flower_list, nfc_node) if (filter->cookie == cls_flower->cookie) break;
...
#define IGB_GSO_PARTIAL_FEATURES (NETIF_F_GSO_GRE |        \
	...
	if (hw->mac.type != e1000_i354)
	{
		dev_info(&pdev->dev, "%s: (PCIe:%s:%s) %pM\n",
				 netdev->name,
				 ((hw->bus.speed == e1000_bus_speed_2500) ? "2.5Gb/s" : (hw->bus.speed == e1000_bus_speed_5000) ? "5.0Gb/s" : "unknown"),
				 ((hw->bus.width == e1000_bus_width_pcie_x4) ? "Width x4" : (hw->bus.width == e1000_bus_width_pcie_x2) ? "Width x2" : (hw->bus.width == e1000_bus_width_pcie_x1) ? "Width x1" : "unknown"), netdev->dev_addr);
	}
	...
	dev_info(&pdev->dev,
			 ...
			 (adapter->flags & IGB_FLAG_HAS_MSIX) ? "MSI-X" : (adapter->flags & IGB_FLAG_HAS_MSI) ? "MSI" : "legacy",
							   ...
							   u16 vlan, u8 qos, __be16 vlan_proto)
{
	...
	return (vlan || qos) ? igb_enable_port_vlan(adapter, vf, vlan, qos) : igb_disable_port_vlan(adapter, vf);
}
...
static inline void igb_vf_reset(struct igb_adapter *adapter, u32 vf)
{
	...
	igb_set_vmvir(adapter, vf_data->pf_vlan | (vf_data->pf_qos << VLAN_PRIO_SHIFT), vf);
...
#else
	unsigned int truesize = ring_uses_build_skb(rx_ring) ? SKB_DATA_ALIGN(IGB_SKB_PAD + size) : SKB_DATA_ALIGN(size);
#endif
